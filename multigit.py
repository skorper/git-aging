...
################################################################################
# Thread worker which does pulls_per_grep pulls on the specified (j) repo

def pullworker(i, j):
	for n in range(i, i + pulls_per_grep):
		checkout_command = 'git pull --no-edit -q -s recursive -X theirs ' + os.getcwd() + "/repos/" + repo_names[j] + ' '  + repo_revs[j][n]
		print(tcolors.pullnumber + str(n).zfill(6) + tcolors.end + ' ' + checkout_command)
		subprocess.check_call(shlex.split(checkout_command), cwd=aged.mntpnt + '/repos/' + repo_names[j], stdout=FNULL, stderr=subprocess.STDOUT)
	return
...
for repo in repos:
	# parse repo name
	repo_name = repo.split(os.sep)[-1].replace('.git', '')
	repo_names.append(repo_name)

	# only clone repo if it doesn't exist
	if not os.path.exists("repos/" + repo_name + "/.git"):
		# clone git repo onto local partition
		git_clone_cmd = "git clone " + repo + " " + "repos/" + repo_name
		print(git_clone_cmd)
		subprocess.check_call(shlex.split(git_clone_cmd));
	else:
		print("git repo " + repo_name + " already exists")

	# delete existing repo on aged (don't want it to effect results)
	print("rm -rf " + aged.mntpnt + "/repos/*")
	subprocess.check_call(shlex.split("rm -rf " + aged.mntpnt + "/repos/*"));

	# create repo dir on aged partition
	print("mkdir -p " + aged.mntpnt + "/repos/" + repo_name)
	subprocess.check_call(shlex.split("mkdir -p " + aged.mntpnt + "/repos/" + repo_name))

	# get repo revision list
	git_rev_list_cmd = "git rev-list --reverse HEAD"
	dir_name = os.getcwd() + "/repos/" + repo_name
	print (dir_name)
	rev_list = subprocess.check_output(shlex.split(git_rev_list_cmd), cwd = dir_name).split('\n')
...
# print("initializing repos on target drive")
# print("mkdir -p " + aged.mntpnt + "/repos")
# subprocess.check_call(shlex.split("mkdir -p " + aged.mntpnt + "/repos"))

# initializing each dir in repos dir [that isn't the repo dir]
for x in glob(aged.mntpnt + '/repos/*/'):
	print("git init " + x)
	subprocess.check_call(shlex.split("git init"), cwd=x, stdout=FNULL, stderr=subprocess.STDOUT)
...
for i in range(0, total_pulls + 1, pulls_per_grep): 
 ...
 # checkout procedure
 jobs = []
 for j in range (0, len(repo_names)):
	 #checkout_command = 'git pull --no-edit -q -s recursive -X theirs ' + os.getcwd() + "/repos/" + repo_names[j] + ' '  + repo_revs[j][i]

	 p = multiprocessing.Process(target=pullworker, args=(i,j,))
	 jobs.append(p)
	 p.start()
 ...
 if i % pulls_per_grep == 0:
  ...
  # clean grep test
  if clean.name != "":
   print("CLEAN.NAME != EMPTY")
   print(tcolors.bold + '\nrunning clean grep test: ' + str(i/pulls_per_grep) + '\n' + tcolors.end)
   subprocess.check_call(shlex.split("bash format.sh clean"))
   print("cp -a " + aged.mntpnt + "/repos " + clean.mntpnt)
   subprocess.check_output(shlex.split("cp -a " + aged.mntpnt + "/repos " + clean.mntpnt))
   cleanresult = False
